...
assert.deepEqual(result.summary, { changes: 6, replaces: 0, inserts: 1, deletes: 5 });
...
assert.deepEqual(result.summary, { changes: 2, replaces: 2, inserts: 0, deletes: 0 });
...
const cmsHeading = `<span class="info24-t2">${pokemonTitle}</span>\n`;
const cmsBefore = `${cmsHeading}<img src="jp_bag.jpg" />\n\n浦和レッズは、8/15(土)サンフレッチェ広島戦にて“ポケモンJリーグフェス”を開催いたします。\n`;
...
assert.equal(result.rows[0].beforeRaw, cmsHeading);
assert.equal(result.rows[0].afterRaw, `◆${pokemonTitle}\n`);
...
assert.ok(result.summary.deletes >= 1);
