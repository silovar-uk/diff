...
- 左右とも空欄のときだけ使い方とサンプル入口を表示。
- 編集中は「よく使う整形」を常時展開。検索・CMSタグ・履歴は初期状態で閉じる。
- 比較中は差分集計・前後移動・GPT・表示を上部へ集約し、左側をレビュー概要へ切り替える。
- 右端の差分マップは各行の位置と種別を表示。クリックまたはJ/Kで移動できる（入力・ダイアログ操作中はJ/K無効）。
- 修正後の文字を選択すると、エディタ上部にCMSタグ操作を表示。
- 全削除は「⋯」内に配置。既存の確認とUndoを維持。
- 767px以下では整形を展開したまま、その他の編集ツールをまとめて折りたたむ。
- HTMLタグ除外は「表示」へ移動し、「表示を反映」で確定。

検証: `npm test`。DOM統合テストは `npm ci` 後に `npm run test:dom`（Node.js 22.13以上）。DOMテストは実際のHTMLと動的読込モジュールも実行し、PC・モバイルの状態分岐を検証します。画面の見た目や実ブラウザ固有の挙動は別途確認が必要です。

## レビュー中心UI（2026-09-22）

- 編集時の左サイドバーを廃止し、整形・検索置換・CMSタグ・履歴を「修正後」エディタ内へ移動。
- 空の状態では編集機能を前面に出さず、「変更前を貼り付け」「修正後を貼り付け」を主操作として表示。
- モード名を「原稿 / レビュー」へ簡略化。
- 長い未変更部分は自動で折りたたみ、変更箇所へスクロールを集中。
- 各差分に「○ / ✓」の確認状態を持ち、確認済み件数をローカル保存。
- 確認状態は差分の内容から作るキーで保持するため、別の差分を修正して再計算しても同じ変更は確認済みを維持。
- 右端の差分マップにも確認済み状態を反映。
- 「次の未確認へ」で未確認の差分だけ順番に追える。
- 「集中」では現在の差分1件だけを表示。J/Kで前後移動、Vで確認済みを切り替え。
- 「最終稿」では差分色やHTMLタグを外し、修正後本文を通常の文章として通読。
- 出力時に未確認件数が残っていれば警告するが、出力自体はブロックしない。
- スマホのレビュー画面は左右2列を廃止し、「変更前 → 修正後」の縦カード表示へ切り替え。
- CIで静的契約テストに加え、jsdomによるデスクトップ/モバイルのDOM統合テストも毎回実行。

## 基本操作

1. 「変更前」と「修正後」にテキストを貼り付けます。
